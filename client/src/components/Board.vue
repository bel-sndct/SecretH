...
<div>
    ...
    <div v-show="waiting" class="waiting messages">
        <div class="infoMessage">
            <img src="../assets/board_icon.png" alt="No icon" class="boardIcon">
            <div>{{message}}</div>
            <button v-show="start" @click="hostStart()" class="startButton">Start Game</button>
        </div>
    </div>

    <v-popup v-if="restart" @restartGame="hostRestart()"/>
...
import VPopup from "./EndingModal";
  ...
  data() {
    ...
    created () {
        ...
        socket.on('playerCount', (playerCount) => {
            ...
            if (Object.keys(playerCount).length < 5) {
