...
io.on('connection', function (socket) {
    ...
    socket.on('createGame', function (data) {
        roomID = randomstring.generate({length: 4, charset: 'alphabetic'});
        socket.join(roomID);
    ...
    socket.on('send_message', function (username, text) {
        let newText = '[Player]' + username + ': ' + text;
...
function assignRoles(players){
    ...
    for (let key in players){
        ...
        io.in(players[key]).emit('getRoleCard', randomAssign);
...
function assignChancellor(roomID){
    io.to(rooms[roomID]['turnOrder'][0]).emit('receive', 'Pick your chancellor');
